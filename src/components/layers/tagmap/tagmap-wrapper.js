...
export default class TagMapWrapper {
  ...
  setData(data, { getLabel, getPosition, getAngle }) {
    this.textSizes = {};
    data.forEach((d) => {
    ...
    this.tagMap.buildHierarchy(data, { getLabel, getPosition, getAngle });
    this.clusterCache = {};
  }
  ...
  getTags({ zoom, bbox, minFontSize, maxFontSize, weightThreshold }) {
    ...
    const cluster =
      this.clusterCache[zoom] || this.extractCluster({ scale, weightThreshold });
